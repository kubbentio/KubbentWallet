...
public class LndMobileScheduledSyncWorker extends ListenableWorker {
  ...
  private final String HANDLERTHREAD_NAME = "kubbent_lndmobile_sync";
  ...
  public LndMobileScheduledSyncWorker(@NonNull Context context, @NonNull WorkerParameters workerParams) {
    ...
    kubbentTor = new KubbentTor(new ReactApplicationContext(getApplicationContext()));
  }
  ...
  private void startLndWorkThread(ResolvableFuture future, String password) {
    ...
    // java.lang.RuntimeException: Can't create handler inside thread Thread[kubbent_lndmobile_sync,5,main] that has not called Looper.prepare()
  ...
  private void stopWorker(boolean success) {
    ...
    if (torStarted) {
      if (!MainActivity.started) {
        ...
        // kubbentTor.stopTor(new PromiseWrapper() {
        //   @Override
  ...
  private boolean startTor() {
    ...
    kubbentTor.startTor(new PromiseWrapper() {
      @Override
      void onSuccess(@Nullable Object value) {
  ...
  private void startLnd() throws RemoteException {
    ...
    if (torEnabled) {
      ...
      int socksPort = KubbentTorUtils.getSocksPort();
      int controlPort = KubbentTorUtils.getControlPort();
  ...
  private boolean checkLndProcessExists() {
    ...
    for (ActivityManager.RunningAppProcessInfo p : am.getRunningAppProcesses()) {
      ...
      if (p.processName.equals(packageName + ":kubbentLndMobile")) {
        HyperLog.d(TAG, "Found " + packageName + ":kubbentLndMobile pid: " + String.valueOf(p.pid));
        return true;
      }
    }
    return false;
  ...
  private boolean killLndProcess() {
    ...
    for (ActivityManager.RunningAppProcessInfo p : am.getRunningAppProcesses()) {
      if (p.processName.equals(packageName + ":kubbentLndMobile")) {
        HyperLog.i(TAG, "Killing " + packageName + ":kubbentLndMobile with pid: " + String.valueOf(p.pid));
