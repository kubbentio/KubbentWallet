...
export interface IKubbentLsp {
  initialize: Thunk<IKubbentLsp, void, IStoreInjections, IStoreModel>;

  // On-demand Channels
  ondemandChannel: IOndemandChannel;
};

export const kubbentLsp: IKubbentLsp = {
  initialize: thunk(async (actions, _, { getState, getStoreState, getStoreActions }) => {
    ...
    LndMobileEventEmitter.addListener("SubscribeChannelEvents", async (e: any) => {
      ...
      if (registerInvoicePreimage) {
        ...
        if (!tx) {
          log.e("Couldn't find transaction while atttempting to settle KubbentLSP invoice", [tx]);
          return;
        }
        tx.status = "SETTLED";
  ...
  ondemandChannel: {
    ...
    addInvoice: thunk((async (actions, { sat, description }, { getStoreActions }) => {
      ...
      const invoice = await getStoreActions().receive.addInvoiceKubbentLsp({
        sat,
        preimage,
